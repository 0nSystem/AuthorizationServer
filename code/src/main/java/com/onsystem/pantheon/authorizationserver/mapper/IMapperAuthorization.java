...
public interface IMapperAuthorization {
    ...
    default Oauth2AuthorizationEntity toEntity(
            final OAuth2Authorization authorization
    ) {
        final Oauth2AuthorizationEntity oAuth2Authorization = new Oauth2AuthorizationEntity();
        ...
        if (accessToken != null) {
            ...
            if (!CollectionUtils.isEmpty(accessToken.getToken().getScopes())) {
                oAuth2Authorization.setAccessTokenScopes(accessToken.getToken().getScopes());
            }
        }
    ...
    default OAuth2Authorization toOAuth2Authorization(Oauth2AuthorizationEntity oAuth2Authorization) {
        ...
        final org.springframework.security.oauth2.server.authorization.OAuth2Authorization.Builder authorizationBuilder = org.springframework.security.oauth2.server.authorization.OAuth2Authorization.withRegisteredClient(registeredClient)
                ...
                .authorizedScopes(new HashSet<>(Arrays.asList(oAuth2Authorization.getAuthorizedScopes())))
        ...
        if (StringUtils.hasText(oAuth2Authorization.getAccessTokenValue())) {
            final OAuth2AccessToken oAuth2AccessToken = new OAuth2AccessToken(
                    ...
                    new HashSet<>(Arrays.asList(oAuth2Authorization.getAuthorizedScopes()))
            );
